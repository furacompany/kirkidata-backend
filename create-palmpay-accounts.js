...
async function createPalmPayAccount(user) {
  try {
    ...
    if (!bvnResult.success) {
      console.log(`⚠️ BVN verification failed for ${user.firstName} ${user.lastName}, skipping`);
      return false;
    }

    // Create virtual account name
    const virtualAccountName = `${user.firstName.trim()} ${user.lastName.trim()}`.replace(/\s+/g, " ").trim();
    ...
    if (!response.data || response.data.respCode !== "00000000") {
      console.log(`❌ PalmPay API error for ${user.firstName} ${user.lastName}:`, response.data?.respMsg);
      return false;
    }

    // Save to database
    const VirtualAccount = mongoose.models.VirtualAccount || mongoose.model("VirtualAccount", new mongoose.Schema({}, { strict: false }));
    ...
    console.log(`✅ Created PalmPay account for ${user.firstName} ${user.lastName}: ${response.data.data.virtualAccountNo}`);
    return true;
  } catch (error) {
    console.log(`❌ Error creating PalmPay account for ${user.firstName} ${user.lastName}:`, error.message);
    return false;
  }
}
...
async function createAccounts() {
  try {
    ...
    for (let i = 0; i < users.length; i++) {
      const user = users[i];
      console.log(`\n📋 Processing user ${i + 1}/${users.length}: ${user.firstName} ${user.lastName}`);
      ...
      if (existingAccount) {
        console.log(`ℹ️ User ${user.firstName} ${user.lastName} already has a PalmPay account, skipping`);
        continue;
      }
